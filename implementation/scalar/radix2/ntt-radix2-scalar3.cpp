#include "ntt-radix2-scalar3.h"

void NTT_radix2_iterative_max_scalar(unsigned *outvec, unsigned *invec, ull n, ull root, ull mod)
{
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        ull v_0 = MOD_2(T_0 * outvec[i_2]);
        ull v_1 = MOD_2(T_1 * outvec[i_3]);
    ...
    for (int len = 8, half = 4; len <= n; len *= 2, half *= 2)
    {
        ...
        for (int i = 0; i < n; i += len)
        {
            for (int j = 0; j < half; j+=4)
            {
                ...
                ull v_0 = MOD_2(T[j  ] * outvec[i_j_half  ]);
                ull v_1 = MOD_2(T[j_1] * outvec[i_j_1_half]);
                ull v_2 = MOD_2(T[j_2] * outvec[i_j_2_half]);
                ull v_3 = MOD_2(T[j_3] * outvec[i_j_3_half]);
