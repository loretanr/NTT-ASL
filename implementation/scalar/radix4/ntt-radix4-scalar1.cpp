...
#include "ntt-radix4-scalar1.h"

using namespace std;

void scalar1_NTT_radix4_base(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    if (isPower4(N))
    {

        scalar1_NTT_rec(N, log4(N), Y, X, 1);
    }
    else
    {
        ull N_div_2 = N >> 1;
        scalar1_NTT_rec(N_div_2, log4(N_div_2), even, X, 2);
        scalar1_NTT_rec(N_div_2, log4(N_div_2), odd, X + 1, 2);
...
void scalar1_NTT_rec(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    if (N == 4)
    {
        scalar1_NTT4_base(Y, X, s);
    }
    else
    {
        for (j = 0; j < 4; j++)
        {
            scalar1_NTT_rec(N_div_4, n - 1, Y + (N_div_4) * j, X + j * s, s * 4);
        }
        for (j = 0; j < N_div_4; j += 4)
        {
            scalar1_NTT4_twiddle(Y + (j + 0), N_div_4, n, j + 0);
            scalar1_NTT4_twiddle(Y + (j + 1), N_div_4, n, j + 1);
            scalar1_NTT4_twiddle(Y + (j + 2), N_div_4, n, j + 2);
            scalar1_NTT4_twiddle(Y + (j + 3), N_div_4, n, j + 3);
        }
    }
}

// s: stride, should always be 1
inline void scalar1_NTT4_base(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
...
inline void scalar1_NTT4_twiddle(unsigned *Y, ull s, ull n, ull j)
{
    ull t0, t1, t2, t3,
        X0, X1, X2, X3;
    ull *Dj;
