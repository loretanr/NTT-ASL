#include "ntt-radix2-avx4.h"

void avx_radix2_iterative_loop_unrolling_mod(unsigned *outvec, unsigned *invec, ull n, ull root, ull mod) {
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        ull v_0 = MOD_2(T_0 * outvec[i + 2]);
        ull v_1 = MOD_2(T_1* outvec[i + 3]);
    ...
    for (int i = 0; i < n; i += 8) {
        ...
        __m128i v_res = _mm128i_VEC_MOD_MULT_2(T_vec,v_vec, mod);
    ...
    for (int len = 16, half = 8; len <= n; len <<= 1, half <<= 1) {
        ...
        for (int i = 0; i < n; i += len) {
            for (int j = 0; j < half; j+=8) {
                ...
                __m128i v_res   = _mm128i_VEC_MOD_MULT_2(T_vec  ,v_vec  , mod);
                __m128i v_res_2 = _mm128i_VEC_MOD_MULT_2(T_vec_2,v_vec_2, mod);
