#include "ntt-radix2-avx2.h"

void avx_radix2_iterative_set_statements(unsigned *outvec, unsigned *invec, ull n, ull root, ull mod) {
    ...
    //__m128i T_vec = _mm_loadu_si128((__m128i *)T[0]);
    ...
    for (int len = 16, half = 8; len <= n; len <<= 1, half <<= 1) {
        ...
        for (int i = 0; i < n; i += len) {
            for (int j = 0; j < half; j+=4) {
                //T_vec = _mm_load_si128((__m128i*)T[j]);
