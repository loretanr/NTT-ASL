...
void avx_NTT_radix4_fma(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    if (isPower4(N))
    {
        avx_NTT_rec_fma(N, log4(N), Y, X, 1);
    }
    else
    {
        ull N_div_2 = N >> 1;
        unsigned log4_N2 = log4(N_div_2);
        avx_NTT_rec_fma(N_div_2, log4_N2, even, X, 2);
        avx_NTT_rec_fma(N_div_2, log4_N2, odd, X + 1, 2);
        ...
        for (int k = 0; k < N_div_2; k+=4)
        {
            ...
            __m128i t = _mm128i_VEC_MOD_MULT_3(w, odd_vec, _mod);
...
void avx_NTT_rec_fma(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    if (N == 4)
    {
        avx_NTT4_base_fma(Y, X, s);
    }
    else if (N == 16)
    {
        avx_NTT16_base_and_twiddle_fma(N, n, Y, X, s);
    }
    else
    {
        for (j = 0; j < 4; j++)
        {
            avx_NTT_rec_fma(N_div_4, n - 1, Y + (N_div_4)*j, X + j * s, s * 4);
        }
        for (j = 0; j < N / 4; j += 4)
        {

            avx_twiddle16_fma(Y + j, N / 4, n, j);
        }
    }
}

inline void avx_NTT16_base_and_twiddle_fma(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    t3 = _mm128i_VEC_MOD_MULT_3(mm128i_baseroot, t3, mod);
    ...
    X1 = _mm128i_VEC_MOD_MULT_3(Y1, D1, mod);
    X2 = _mm128i_VEC_MOD_MULT_3(Y2, D2, mod);
    X3 = _mm128i_VEC_MOD_MULT_3(Y3, D3, mod);
...
inline void avx_twiddle16_fma(unsigned *Y, ull s, ull n, ull j)
{
    ...
    X0 = _mm128i_VEC_MOD_MULT_3(Y0, D0, mod);
    X1 = _mm128i_VEC_MOD_MULT_3(Y1, D1, mod);
    X2 = _mm128i_VEC_MOD_MULT_3(Y2, D2, mod);
    X3 = _mm128i_VEC_MOD_MULT_3(Y3, D3, mod);
...
inline void avx_NTT4_base_fma(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
