...
#include "ntt-radix4-avx-sawtooth.h"
using namespace std;
...
void avx_NTT_radix4_sawtooth(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    if (isPower4(N))
    {
        avx_NTT_rec_sawtooth(N, log4(N), Y, X, 1);
    }
    else
    {
        ull N_div_2 = N >> 1;
        unsigned log4_N2 = log4(N_div_2);
        avx_NTT_rec_sawtooth(N_div_2, log4_N2, even, X, 2);
        avx_NTT_rec_sawtooth(N_div_2, log4_N2, odd, X + 1, 2);
...
void avx_NTT_rec_sawtooth(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    if (N == 4)
    {
        avx_NTT4_base_sawtooth(Y, X, s);
    }
    else if (N == 16)
    {
        avx_NTT16_base_and_twiddle_sawtooth(N, n, Y, X, s);
    }
    else
    {
        for (j = 0; j < 4; j++)
        {
            avx_NTT_rec_sawtooth(N_div_4, n - 1, Y + (N_div_4)*j, X + j * s, s * 4);
        }
        for (j = 0; j < N / 4; j += 4)
        {

            avx_twiddle16_sawtooth(Y + j, N / 4, n, j);
        }
    }
}

inline void avx_NTT16_base_and_twiddle_sawtooth(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
...
inline void avx_twiddle16_sawtooth(unsigned *Y, ull s, ull n, ull j)
{
...
inline void avx_NTT4_base_sawtooth(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
