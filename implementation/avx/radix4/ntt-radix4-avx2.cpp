...
#include "ntt-radix4-avx2.h"
#include <iostream>

using namespace std;

__m128i minus1 = _mm_set1_epi16(-1);
__m128i vec_baseroot = _mm_set1_epi32(baseroot);

void avx2_NTT_radix4_base(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    vec_baseroot = _mm_set1_epi32(baseroot);
    if (isPower4(N))
    {
        if (N < 16)
        {
            avx2_NTT_rec(N, log4(N), Y, X, 1);
        }
        else
        {
            avx2_NTT_rec(N, log4(N), Y, X, 1);
    ...
    else
    {
        ...
        if (N < 16)
        {
            avx2_NTT_rec(N2, log4(N2), even, X, 2);
            avx2_NTT_rec(N2, log4(N2), odd, X + 1, 2);
        }
        else
        {
            avx2_NTT_rec(N2, log4(N2), even, X, 1);
            avx2_NTT_rec(N2, log4(N2), odd, X + N2, 1);
        }
        // twiddle 2
        __m128i w;
...
void avx2_NTT_rec(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    else if (N == 16)
    {
        ...
        t3 = _mm128i_VEC_MOD_MULT(vec_baseroot, t3, mod);
        ...
        _mm_maskstore_epi32((int *)Y + (N >> 2) * 0, minus1, Y0);
        _mm_maskstore_epi32((int *)Y + (N >> 2) * 1, minus1, Y1);
        _mm_maskstore_epi32((int *)Y + (N >> 2) * 2, minus1, Y2);
        _mm_maskstore_epi32((int *)Y + (N >> 2) * 3, minus1, Y3);
    }
    ...
    else
    {
        for (j = 0; j < 4; j++)
        {
            avx2_NTT_rec(N_div_4, n - 1, Y + (N_div_4)*j, X + j * 16 * s, s * 4);
        }
        for (j = 0; j < N / 4; j += 4)
        {
            avx2_twiddle16(Y + j, N / 4, n, j);
        }
    }
}

inline void avx2_NTT4_base(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
...
inline void avx2_twiddle16(unsigned *Y, ull s, ull n, ull j)
{
