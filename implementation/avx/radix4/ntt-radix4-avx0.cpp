...
void avx0_NTT_radix4_base(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    if (isPower4(N))
    {
        ...
        avx0_NTT_rec(N, log4(N), Y, X, 1);
    }
    else
    {
        ull N_div_2 = N >> 1;
        avx0_NTT_rec(N_div_2, log4(N_div_2), even, X, 2);
        avx0_NTT_rec(N_div_2, log4(N_div_2), odd, X + 1, 2);
...
void avx0_NTT_rec(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    else
    {
        for (j = 0; j < 4; j++)
        {
            avx0_NTT_rec(N_div_4, n - 1, Y + (N_div_4)*j, X + j * s, s * 4);
        }
        for (j = 0; j < N / 4; j+=4)
        {
            //avx0_NTT4_twiddle(Y + 1 * j, N / 4, n, j);
            avx0_twiddle16(Y + 1 * j, N / 4, n, j);
        }
    }
}

inline void avx0_NTT4_base(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
...
inline void avx0_twiddle16(unsigned *Y, ull s, ull n, ull j)
{
...
inline void avx0_NTT4_quad_twiddle(__m256d &D0, __m256d &D1, __m256d &D2, __m256d &D3, __m256d &Y0, __m256d &Y1, __m256d &Y2, __m256d &Y3)
{
    __m256d X0, X1, X2, X3;
    __m256d t0, t1, t2, t3;
...
inline void avx0_NTT4_twiddle(unsigned *Y, ull s, ull n, ull j)
{
    ull t0, t1, t2, t3,
        X0, X1, X2, X3;
    ull *Dj;
