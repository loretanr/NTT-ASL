...
void avx4_NTT_radix4_base(unsigned *Y, unsigned *X, ull N, ull _root, ull _mod)
{
    ...
    else
    {
        ...
        if (N < 16)
        {
            avx4_NTT_rec(N2, log4(N2), even, X, 2);
            avx4_NTT_rec(N2, log4(N2), odd, X + 1, 2);
        }
        else
        {
            avx4_NTT_rec(N2, log4(N2), even, X, 1);
            avx4_NTT_rec(N2, log4(N2), odd, X + N2, 1);
        }
        // twiddle 2
        __m128i w;
...
void avx4_NTT_rec(ull N, ull n, unsigned *Y, unsigned *X, ull s)
{
    ...
    else
    {
        for (j = 0; j < 4; j++)
        {
            avx4_NTT_rec(N_div_4, n - 1, Y + (N_div_4)*j, X + j * 16 * s, s * 4);
        }
        for (j = 0; j < N / 4; j += 4)
        {
            avx4_twiddle16(Y + j, N / 4, n, j);
        }
    }
}

inline void avx4_NTT4_base(unsigned *Y, unsigned *X, ull s)
{
    ull X0 = X[0 * s];
    ull X1 = X[1 * s];
    ull X2 = X[2 * s];
...
inline void avx4_twiddle16(unsigned *Y, ull s, ull n, ull j)
{
