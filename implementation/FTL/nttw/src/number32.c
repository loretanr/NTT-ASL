...
void rearrange_custom(unsigned *data, ull n)
{
    size_t target = 0, position = 0, mask = n;
...
// custom for ull
ull bigshr_custom (ull *d, ull *s, ull n)
{
    ull t;
    ull tmp;
    ull b = 0;
...
// custom for ull
ull pow_mod_custom(ull base, ull exp, ull MOD)
{
    ull r;

    if (exp == 0) return 1;

    while (!bigshr_custom (&exp, &exp, 1))
        base = (base * base)%MOD;
...
/* wrapper function to call this is below */

///Cooley-Tukey NTT
inline void fntt(unsigned *outvec,unsigned *data, ull nn, ull pr, ull MOD)
{    
    ull m, i, j, istep, mmax;
    ull w, wt, wr, wtemp;

    w = pow_mod_custom(pr, (MOD - 1) / nn, MOD);

    rearrange_custom(data, nn);

    mmax = 1;
    while (nn > mmax)
    {
        istep = mmax << 1;
        wr = wt = pow_mod_custom(w, nn / istep, MOD);
        ...
        for (i = 0; i < nn; i += istep)
        {
            j = i + mmax;
            wtemp = data[j];
            data[j] = MODSUB(data[i], wtemp, MOD);
            data[i] = MODADD(data[i], wtemp, MOD);
        }
...
// wrapper function
void fntt_wrapper(unsigned *outvec, unsigned *invec, ull n, ull prime, ull root){
    fntt(outvec, invec, n, prime, root);
}
