...
__m256d VEC_MOD_MULT_stackover(__m256d a, __m256d b, int mod)
{   // buggy
    __m256i _a = _mm256_castpd_si256(a);
    __m256i _b = _mm256_castpd_si256(b);
    __m256 _k = _mm256_set1_ps(1.0f / mod);
    __m256i _p = _mm256_set1_epi32(mod);
    __m256i _d = _mm256_mullo_epi32(_a, _b);
    __m256i _e = _mm256_cvtps_epi32(_mm256_mul_ps(_mm256_cvtepi32_ps(_d), _k)); // e = int(float(d)/float(p));
    __m256i _c = _mm256_sub_epi32(_d, _mm256_mullo_epi32(_e, _p));
    return _mm256_castsi256_pd(_c);
}

__m256d VEC_MOD_MULT(__m256d a, __m256d b, int mod)
{   // this would only be mod, not mod-mult!
    __m256d mod_vec = _mm256_set1_pd((double) mod);
    mod_vec = _mm256_set1_pd(2.0);
    __m256d div_vec = _mm256_div_pd(a,mod_vec);

    __m256d vec = _mm256_mul_pd(mod_vec, div_vec);
    return _mm256_sub_pd(a,vec);
}
